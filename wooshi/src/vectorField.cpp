...
//------------------------------------------------------------------------------------
void vectorField::pushLeft(float pushAmount){
    for (int i = 0; i < fieldSize; i++){
        
        field[i].x -= ofNoise(ofGetElapsedTimef()/10*i,0)*pushAmount;
        field[i].y += (ofNoise(0,ofGetElapsedTimef()/10*i)-.5)*pushAmount/3;
    }
}
